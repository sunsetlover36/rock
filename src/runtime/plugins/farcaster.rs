...
use rock_wire::farcaster::{Fid, GetUserByUsernameParams, GetUsersByFidsParams, SendCastParams};
...
mod rx;
use rx::{CastRx, CastRxOpcodes, CastRxParams};
...
impl GameModePlugin for FarcasterPlugin {
    ...
    fn create_scene_api(&self, lua: &mlua::Lua) -> mlua::Result<Option<mlua::Table>> {
        ...
        let cast_fn = lua.create_function(move |_, signer: String| {
            Ok(CastRx::new(CastRxParams {
        ...
        let user_fn = lua.create_function(move |_, v: mlua::Value| {
            ...
            match v {
                ...
                mlua::Value::Table(arr) => {
                    if arr.is_empty() {
                        return Err(mlua::Error::runtime(format!(
                            "{plugin_name}.user: fids list is empty"
                        )));
                    }
                ...
                _ => {
                    return Err(mlua::Error::runtime(format!(
                        "{plugin_name}.user: unknown argument type, expected a string or a table"
                    )));
                }
            }
